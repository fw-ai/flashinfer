...
if __name__ == "__main__":
    ...
    print(f"\n\nUsing device: {device}")
    print(
        "Testing NVFP4 format: Global Scale (FP32) + Block Scale (FP8 E4M3) + Data (E2M1)"
    )
    ...
    print("QUANTIZATION")
    print("-" * 80)
    quant_kv, block_scales_kv, global_scale_kv = KVFP4QuantizeUtil.batched_quantize(
        kv_cache_flat
    )
    ...
    print(
        f"  Block scales shape: {block_scales_kv.shape}, dtype: {block_scales_kv.dtype}"
    )
    print(
        f"  Global scale: {global_scale_kv.item():.6f} (dtype: {global_scale_kv.dtype})"
    )
    print(
        f"  Block scales range: [{block_scales_kv.float().min().item():.6f}, {block_scales_kv.float().max().item():.6f}]"
    )
    ...
    print("-" * 80)
    dequant_kv = KVFP4QuantizeUtil.batched_dequantize(
        quant_kv, block_scales_kv, global_scale_kv
    )
    dequant_kv_reshaped = dequant_kv.reshape(
        num_pages, num_kv, num_kv_heads, page_size, head_dim
    )
    print(
        f"Dequantized KV cache shape: {dequant_kv_reshaped.shape}, dtype: {dequant_kv_reshaped.dtype}"
    )
    ...
    print(
        f"{'Min value':<30} {orig_min:<20.8f} {dequant_min:<20.8f} {dequant_min - orig_min:<20.8f}"
    )
    print(
        f"{'Max value':<30} {orig_max:<20.8f} {dequant_max:<20.8f} {dequant_max - orig_max:<20.8f}"
    )
    print(
        f"{'Mean value':<30} {orig_mean:<20.8f} {dequant_mean:<20.8f} {dequant_mean - orig_mean:<20.8f}"
    )
    print(
        f"{'Std deviation':<30} {orig_std:<20.8f} {dequant_std:<20.8f} {dequant_std - orig_std:<20.8f}"
    )
    print(
        f"{'Range (max-min)':<30} {orig_max - orig_min:<20.8f} {dequant_max - dequant_min:<20.8f} {(dequant_max - dequant_min) - (orig_max - orig_min):<20.8f}"
    )
    ...
    for p in percentiles:
        ...
        print(
            f"{p:>3}th{'':<10} {orig_p:<20.8f} {dequant_p:<20.8f} {dequant_p - orig_p:<20.8f}"
        )
    ...
    print("\nRelative Error:")
    print(
        f"  Max:    {rel_error.max().item():.6f} ({rel_error.max().item() * 100:.2f}%)"
    )
    print(
        f"  Mean:   {rel_error.mean().item():.6f} ({rel_error.mean().item() * 100:.2f}%)"
    )
    print(
        f"  Median: {rel_error.median().item():.6f} ({rel_error.median().item() * 100:.2f}%)"
    )
    ...
    for i in range(num_bins):
        ...
        print(
            f"[{bin_start:>8.4f}, {bin_end:>8.4f}){'':<3} {orig_count:<15.0f} {dequant_count:<15.0f} {diff:>+15.0f}"
        )
    ...
    print(
        f"Test tensor range: [{test_tensor.min().item():.6f}, {test_tensor.max().item():.6f}]"
    )
    ...
    quant1, block_scales1, global_scale1 = KVFP4QuantizeUtil.batched_quantize(
        test_tensor
    )
    dequant1 = KVFP4QuantizeUtil.batched_dequantize(
        quant1, block_scales1, global_scale1
    )
    ...
    print(
        f"  Range: [{block_scales1_float.min().item():.6f}, {block_scales1_float.max().item():.6f}]"
    )
    ...
    print(
        f"  Range: [{dequant1_flat.min().item():.8f}, {dequant1_flat.max().item():.8f}]"
    )
    ...
    print("\n[Storage]")
    print(
        f"  Data: {size1_data} bytes, Block scales: {size1_block} bytes, Global: {size1_global} bytes"
    )
    ...
    print(
        f"  Range: [{test_tensor_flat.min().item():.8f}, {test_tensor_flat.max().item():.8f}]"
    )
    ...
    print(
        f"{'Min':<25} {test_tensor_flat.min().item():<20.8f} {dequant1_flat.min().item():<20.8f}"
    )
    print(
        f"{'Max':<25} {test_tensor_flat.max().item():<20.8f} {dequant1_flat.max().item():<20.8f}"
    )
    print(
        f"{'Mean':<25} {test_tensor_flat.mean().item():<20.8f} {dequant1_flat.mean().item():<20.8f}"
    )
    print(
        f"{'Std':<25} {test_tensor_flat.std().item():<20.8f} {dequant1_flat.std().item():<20.8f}"
    )
    print(
        f"{'Median':<25} {test_tensor_flat.median().item():<20.8f} {dequant1_flat.median().item():<20.8f}"
    )
    ...
    print(
        f"{'Block scale (mean)':<25} {'N/A':<20} {block_scales1_float.mean().item():<20.6f}"
    )
    print(
        f"{'Block scale (range)':<25} {'N/A':<20} {f'[{block_scales1_float.min().item():.2f}, {block_scales1_float.max().item():.2f}]':<20}"
    )
    ...
    for i in range(10):
        ...
        bin_end = hist_min + (i + 1) * bin_width
        print(
            f"[{bin_start:>7.3f}, {bin_end:>7.3f}) {hist_orig[i].item():<15.0f} {hist_dq1[i].item():<15.0f}"
        )
    ...
    print(
        f"{'Data format':<40} {'uint8 (packed E2M1)':<25} {'float32 (E2M1 values)':<25}"
    )
